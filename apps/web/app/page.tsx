...
export default function Home() {
  ...
  async function callModel(connection: Pick<Connection, 'id' | 'name' | 'endpoint' | 'apiKey'>, model: string, content: string, temperature = 0.7, signal?: AbortSignal) {
  ...
  return (
    <main className="shell">
      ...
      <section id="arena" className="arena-card">
        ...
        <div className="run-row"><div className="judge-note"><span className="shield">♢</span><span><strong>默认隐私模式</strong><small>快速竞技刷新即清空；批量模式仅保留不含题目、答案和凭据的脱敏恢复点。</small></span></div><button className="run-button" onClick={runArena} disabled={running}>{running ? '竞技运行中…' : '开始匿名竞技'} <span>→</span></button></div>
      </section>
