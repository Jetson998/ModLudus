...
export default function TrustedSeason() {
  ...
  return <section id="trusted-season" className="trusted-section">
    <div className="trusted-heading"><div><span className="section-kicker">M3.3 / DURABLE TRUSTED SEASON</span><h2>可恢复的服务端可信赛季</h2><p>固定运行清单进入持久任务队列，由独立 Worker 领取和续租；API 重启不会丢失任务。用户浏览器中的 Base URL、Key 和题目仍不会进入可信链路。</p></div><span className={serverStatus?.ready ? 'trust-pill ready' : 'trust-pill'}>{serverStatus?.ready ? `执行器已就绪 · ${serverStatus.environment}${serverStatus.simulated ? ' · simulated' : ''}` : '等待服务端配置'}</span></div>
