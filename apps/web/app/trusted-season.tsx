...
function downloadEvidence(evidence: Evidence) {
  const content = JSON.stringify({
    schema_version: 'm3.2-signed-envelope',
...
export default function TrustedSeason() {
  ...
  const loadEvidence = useCallback(async (runId: string) => {
    const [nextEvidence, nextVerification, audit] = await Promise.all([
  ...
  return <section id="trusted-season" className="trusted-section">
    <div className="trusted-heading"><div><span className="section-kicker">M3.2 / TRUSTED SEASON</span><h2>服务端可信赛季</h2><p>固定测试集、Rubric、候选模型与价格快照由服务器冻结执行。用户在浏览器填写的 Base URL 和 Key 不会发送到这里；可信赛季只使用管理员部署时配置的服务端凭据。</p></div><span className={serverStatus?.ready ? 'trust-pill ready' : 'trust-pill'}>{serverStatus?.ready ? `执行器已就绪 · ${serverStatus.environment}${serverStatus.simulated ? ' · simulated' : ''}` : '等待服务端配置'}</span></div>
    ...
    <div className="trust-overview">
      ...
      <article><small>审计链</small><strong>{serverStatus ? (serverStatus.audit_chain_valid ? '有效' : '异常') : '待检查'}</strong><span>Append-only 哈希链与不可变证据表</span></article>
    </div>

    <article className="trusted-control">
      <div><strong>可信运行控制台</strong><p>{message}</p>{serverStatus?.configuration && <small>候选：{serverStatus.configuration.candidates.join('、')} · 裁判：{serverStatus.configuration.judge} · 题目并发 {serverStatus.configuration.concurrency} · 写入鉴权 {serverStatus.start_auth.mode}</small>}{serverStatus?.start_auth.mode === 'admin-token' && <input className="admin-token-input" type="password" value={adminToken} onChange={(event) => setAdminToken(event.target.value)} placeholder="管理员启动令牌（仅保存在页面内存）" aria-label="可信赛季管理员启动令牌" />}</div>
    ...
    <div className="trusted-grid">
      <article className="run-history"><div className="panel-title"><div><strong>运行记录</strong><small>服务器持久化状态；证据一旦封存不可更新或删除</small></div></div>{runs.length ? runs.map((run) => <button key={run.id} className={activeRun?.id === run.id ? 'run-history-item selected' : 'run-history-item'} onClick={() => void selectRun(run)}><span><strong>{run.id.slice(-12)}</strong><small>{run.created_at.replace('T', ' ').slice(0, 19)} UTC · {run.environment}{run.simulated ? ' · simulated' : ''}</small></span><em>{run.status} · {run.completed_cases}/{run.total_cases}</em></button>) : <p className="empty-copy">尚无可信赛季运行。</p>}</article>

      <article className="evidence-panel"><div className="panel-title"><div><strong>签名报告</strong><small>证据哈希、Ed25519 签名、公钥及冻结运行清单</small></div>{verification && <span className={verification.verified ? 'verified-chip' : 'verified-chip invalid'}>{verification.verified ? '✓ 验签通过' : '验签失败'}</span>}</div>{evidence ? <><div className="evidence-origin"><span>签名证据环境</span><strong>{evidence.report.manifest.environment ?? activeRun?.environment ?? 'legacy-unfrozen'}{(evidence.report.manifest.simulated ?? Boolean(activeRun?.simulated)) ? ' · simulated' : ''}</strong></div><div className="evidence-hashes"><span>Evidence <code>{shortHash(evidence.evidence_hash)}</code></span><span>Dataset <code>{shortHash(evidence.report.manifest.dataset_hash)}</code></span><span>Rubric <code>{shortHash(evidence.report.manifest.rubric.fingerprint)}</code></span><span>Config <code>{shortHash(evidence.report.manifest.configuration_hash)}</code></span></div><div className="evidence-summary"><div><small>完成题目</small><strong>{evidence.report.summary.total_cases}</strong></div><div><small>需复核</small><strong>{evidence.report.summary.review_required}</strong></div><div><small>失败调用</small><strong>{evidence.report.summary.failed_attempts}</strong></div></div><button className="outline-button" onClick={() => downloadEvidence(evidence)}>下载签名 JSON 报告</button></> : <p className="empty-copy">运行完成后生成不可变签名报告。</p>}</article>
    </div>
