...
class EvidenceStore:
    ...
    def create_run(self, run_id: str, manifest_hash: str, environment: str, simulated: bool) -> None:
        ...
        self.append_audit(run_id, "run.created", {"manifest_hash": manifest_hash, "season_id": SEASON_ID, "environment": environment, "simulated": simulated})
    ...
    def save_evidence(self, run_id: str, report: dict[str, Any], signer: EvidenceSigner) -> dict[str, Any]:
        ...
        self.update_run(run_id, evidence_id=evidence_id)
        self.append_audit(run_id, "evidence.sealed", {"evidence_id": evidence_id, "evidence_hash": evidence_hash})
...
def build_manifest(config: TrustedConfig, configuration_salt: str, environment: str, simulated: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "execution": {"case_concurrency": config.concurrency, "candidate_concurrency": len(config.candidates)},
    }
...
async def execute_trusted_run(
    ...
    store.append_audit(run_id, "run.started", {"manifest_hash": sha256_text(canonical_json(manifest))})
    ...
    try:
        ...
        for task in asyncio.as_completed(tasks):
            ...
            store.append_audit(run_id, "case.completed", {"case_id": result["case_id"], "completed_cases": len(results)})
        ...
        for result in results:
            ...
            for attempt in result["attempts"]:
                summary = model_summary.setdefault(attempt["model"], {"model": attempt["model"], "scores": [], "wins": 0, "failures": 0, "latencies": [], "cost_usd": 0.0})
                if attempt["failed"]:
                    summary["failures"] += 1
                else:
                    summary["latencies"].append(attempt["latency_ms"])
                    summary["cost_usd"] += attempt["estimated_cost_usd"]
                    if attempt["alias"] in score_by_alias:
                        summary["scores"].append(score_by_alias[attempt["alias"]])
                    if result["judge"] and result["judge"]["winner"] == attempt["alias"]:
                        summary["wins"] += 1
        ranking = [{
            "model": item["model"],
        ...
        report = {
            "schema_version": "m3.2", "run_id": run_id, "season_id": SEASON_ID,
        ...
        store.update_run(run_id, status="failed", completed_at=utc_now(), error=f"{type(error).__name__}: {error}")
...
class TrustedSeasonRuntime:
    ...
    def _close_interrupted_runs(self) -> None:
        for run in self.store.list_runs(100):
            if run["status"] not in {"queued", "running"}:
                continue
            self.store.update_run(run["id"], status="failed", completed_at=utc_now(), error="api_restart_interrupted")
            self.store.append_audit(run["id"], "run.interrupted", {"reason": "api_restart_interrupted"})
    ...
    def start_run(self) -> dict[str, Any]:
        ...
        task = asyncio.create_task(execute_trusted_run(run_id, self.config, manifest, self.store, self.signer))
        self.tasks.add(task)
        task.add_done_callback(self.tasks.discard)
        return self.store.get_run(run_id) or {}

    def start_authorized(self, presented_token: str, client_host: str) -> bool:
        if self.admin_token and presented_token and secrets.compare_digest(self.admin_token, presented_token):
            return True
